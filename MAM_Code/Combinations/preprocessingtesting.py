...
class DataPreprocessortesting:
    def __init__(self,means,anomalies):
        self.means = means
    ...
    def calculate_monthly_anomalies(self, data,means,index):
        anomalies = np.zeros(data.shape)
        means_adjusted = np.roll(means,-2, axis=1)
        if index == 0:
            adjusted_means = means[:, :, :data.shape[2]]
            anomalies = data - adjusted_means
    ...
    def min_max_normalize(self, data, anomalies):
        anomalies_adjusted = np.roll(anomalies,-2, axis=1) 
        normalized_data = np.zeros(data.shape)
        for i in range(anomalies_adjusted.shape[2]):  
            for j in range(anomalies_adjusted.shape[1]):
                

                anomaly_month = anomalies_adjusted[:, j, i] 
                min_value = np.min(anomaly_month)  
                max_value = np.max(anomaly_month)  
                month = data[:, j, i]  
    ...
    def execute_pipeline(self, nc_file1, nc_file2):
        ...
        anomalies1 = self.calculate_monthly_anomalies(coarsed_data1,self.means,0)
        anomalies2 = self.calculate_monthly_anomalies(coarsed_data2,self.means,1)
        
        concatenated_anomalies = np.concatenate((anomalies1, anomalies2), axis=2)

        normalized_data = self.min_max_normalize(concatenated_anomalies,self.anomalies)
        return normalized_data
