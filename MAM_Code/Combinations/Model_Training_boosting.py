...
class ENSOAnalysis_br:
    ...
    def plotting(self,y_test, y_pred, month, corr_coef, features,corr_name):
        ...
        plt.title(f'Air_T + GPH {month} with {corr_name} correaltion and {features} features', fontsize=14)
        plt.xlim(-2, 2)
        plt.ylim(-2, 2)
        plt.grid(True)
        plt.legend()
        folder_name = "Boosting_Correlation"
    ...
    def plotting_y_pred_vs_y_test(self, y_test, y_pred, month):
        ...
        ax.set_title(f'Air_T + GPH {month} Actual vs Predicted')
        ax.legend()
        ax.set_xticks(years)
