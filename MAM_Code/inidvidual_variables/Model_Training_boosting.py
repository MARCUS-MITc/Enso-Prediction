...
class ENSOAnalysis_br:
    ...
    def processing(self,csv,correaltion,features,month):
        df = pd.read_csv(csv)
        df['mam_Avg'] = df[['Mar', 'Apr','May']].mean(axis=1)
    ...
    def tune_gradient_boosting_regressor(self, X_train, y_train):
        ...
        param_grid = {
            ...
            'max_depth': range(3, 8),
            #'min_samples_split': range(2, 21),  # Adjust range as needed
            'min_samples_leaf': range(1, 11)   # Adjust range as needed
        }
    
        # Use HistGradientBoostingRegressor for potential performance gains
        gradient_boosting_reg = HistGradientBoostingRegressor(random_state=42)
        grid_search = RandomizedSearchCV(gradient_boosting_reg, param_grid,
                                         n_iter=300, random_state=42, cv=3, scoring='r2', n_jobs=-1)
        with warnings.catch_warnings():
            warnings.filterwarnings("ignore", category=UserWarning)
            grid_search.fit(X_train, y_train)
    ...
    def model_bag_reg(self,best_model, X_test, y_test):
        ...
        correlation_ = np.corrcoef(y_pred, y_test)[0, 1]
        return correlation_, y_pred
    ...
    def plotting(self,y_test, y_pred, month, corr_coef, features,corr_name):
        ...
        plt.plot([-2, 2], [-2, 2], color='red', linestyle='--', label=f'correlation: {corr_coef:.2f}')
        plt.xlabel('Actual Values', fontsize=12)
        plt.ylabel('Predicted Values', fontsize=12)
        plt.title(f'Air_T {month} with {corr_name} correaltion and {features} features', fontsize=14)
        plt.xlim(-2, 2)
        plt.ylim(-2, 2)
        plt.grid(True)
        plt.legend()
    ...
    def plotting_y_pred_vs_y_test(self, y_test, y_pred, month):
        ...
        ax.set_title(f'Air_T {month} Actual vs Predicted')
        ax.legend()
        ax.set_xticks(years)
        ...
        for m in month:
            ...
            for f in features:
                for corr_name, correaltion in zip(correlation_methods, correlation_dfs):
                    ...
                    current_model, _, _ = self.tune_gradient_boosting_regressor(X_train, y_train)
            ...
            print(f"vwind {m}, {max_corr_name} correlation {max_corr_feature} features highest correlation coefficient: {max_corr:.2f}")
...
#             best_model, _, _ = self.tune_bagging_regressor(X_train, y_train)
            correlation_coeff, _ = self.model_bag_reg(best_model, X_test, y_test)
        ...
        return self.best_models,self.scalers
