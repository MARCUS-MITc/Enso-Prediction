...
#from Long_term_anomalies import DataPreprocessor


class DataPreprocessortesting:
    def __init__(self, nc_file,means,anomalies):
    ...
    def calculate_monthly_anomalies(self, data,means):
        anomalies = np.zeros(data.shape)
        means_adjusted = np.roll(means,-3, axis=1) 
        anomalies = data - means_adjusted
        return anomalies
    ...
    def execute_pipeline(self):
        ...
        anomaly = self.calculate_monthly_anomalies(coarsed_data,self.means)
        
        normalized_data = self.min_max_normalize(anomaly,self.anomalies)
